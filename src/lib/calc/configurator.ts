...
export function generatePositions(input: ConfiguratorInput): CalcPosition[] {
  ...
  if (moduleProd && kwp > 0) {
    ...
    const count = wp > 0 ? Math.ceil((kwp * 1000) / wp) : 0;
    if (count > 0) out.push(makePosition(moduleProd, count, "PV-Anlage", kwp));
  }

  // 2) Wechselrichter: Kategorie/Name, nach Hersteller-Präferenz. Menge 1.
  const inverterCandidates = byPref(
    products.filter((p) => matches(p, ["wechselrichter", "inverter", "controller"]) && (num(specsOf(p).module_wp) ?? 0) === 0),
    prefs?.inverterManufacturer,
  );
  if (inverterCandidates[0]) out.push(makePosition(inverterCandidates[0], 1, "PV-Anlage", kwp));
  ...
  // 4) Dienstleistungen (is_service): nach kWp-Staffel bepreist, Menge 1.
